import { HEIGHEST_PRICE, HEIGHEST_RATING, SORT_OPTIONS } from "@/components/product/sort-product"
...
const sortProduct = ({ a, b, sortOption }: { a: Product, b: Product, sortOption: string }): number => {
...
const useProductController = (productsData: Product[]) => {
  ...
  const filteredAndSortedProduct = useMemo(() => {
    ...
    if (filterText && sortOption) {
      return products.filter(p => p.title.toLowerCase().includes(filterText)).sort((a, b) => sortProduct({ a, b, sortOption }))
    }
